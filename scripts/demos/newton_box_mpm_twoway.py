...
CoupledNewtonCfg = MJWarpSolverCfg = MPMSolverCfg = None
NewtonManager = NewtonProxyCoupledMJWarpMPMManager = ProxyCoupledMJWarpMPMSolverCfg = None
...
def import_runtime_dependencies() -> None:
    ...
    global CoupledNewtonCfg, MJWarpSolverCfg, MPMSolverCfg
    global NewtonManager, NewtonProxyCoupledMJWarpMPMManager, ProxyCoupledMJWarpMPMSolverCfg
    ...
    from isaaclab_contrib.deformable.newton_manager_cfg import (
        CoupledNewtonCfg as CoupledNewtonCfgClass,
    )
    from isaaclab_contrib.deformable.newton_manager_cfg import (
        MPMSolverCfg as MPMSolverCfgClass,
    )
    from isaaclab_contrib.deformable.newton_manager_cfg import (
        ProxyCoupledMJWarpMPMSolverCfg as ProxyCoupledMJWarpMPMSolverCfgClass,
    )
    from isaaclab_contrib.deformable.proxy_coupled_mjwarp_mpm_manager import (
        NewtonProxyCoupledMJWarpMPMManager as NewtonProxyCoupledMJWarpMPMManagerClass,
    )
    ...
    CoupledNewtonCfg = CoupledNewtonCfgClass
    MJWarpSolverCfg = MJWarpSolverCfgClass
    MPMSolverCfg = MPMSolverCfgClass
    NewtonManager = NewtonManagerClass
    NewtonProxyCoupledMJWarpMPMManager = NewtonProxyCoupledMJWarpMPMManagerClass
    ProxyCoupledMJWarpMPMSolverCfg = ProxyCoupledMJWarpMPMSolverCfgClass
...
def build_box_sand_model() -> tuple[newton.ModelBuilder, ProxyCoupledMJWarpMPMSolverCfg, int]:
    ...
    solver_cfg = ProxyCoupledMJWarpMPMSolverCfg(
        ...
        mjwarp_bodies=[BOX_BODY_PATH],
        mpm_bodies=[],
        proxy_bodies=[BOX_BODY_PATH],
        proxy_mode="lagged",
        proxy_iterations=args_cli.proxy_iterations,
        proxy_mass_scale=args_cli.proxy_mass_relaxation,
    )
...
def read_proxy_wrench(box_body: int, dt: float) -> torch.Tensor:
    """Return the latest sand reaction wrench by reading MPM collider impulses directly."""
    state = NewtonManager.get_state_0()
    zero_device = wp.to_torch(state.joint_q).device if state.joint_q is not None else "cpu"

    try:
        mpm_solver = NewtonProxyCoupledMJWarpMPMManager.get_entry_solver(SAND_ENTRY)
    except (KeyError, RuntimeError):
        return torch.zeros(6, device=zero_device)
    if not (hasattr(mpm_solver, "collect_collider_impulses") and hasattr(mpm_solver, "collider_body_index")):
        return torch.zeros(6, device=zero_device)
...
def create_launcher_sim_cfg():
    ...
    from isaaclab_contrib.deformable.newton_manager_cfg import CoupledNewtonCfg as CoupledNewtonCfgClass
    ...
    return sim_utils_module.SimulationCfg(
        ...
        physics=CoupledNewtonCfgClass(num_substeps=1, use_cuda_graph=not args_cli.disable_cuda_graph),
    )
...
def main() -> None:
    ...
    with launch_simulation(SimpleNamespace(sim=sim_cfg), args_cli):
        ...
        sim_cfg.physics = CoupledNewtonCfg(
