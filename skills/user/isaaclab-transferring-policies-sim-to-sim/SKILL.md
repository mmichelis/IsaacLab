...
3. **Articulation joint and body ordering.** A checkpoint stores column positions, not names, so a cross-backend replay with mismatched axis order feeds each joint's observation to a different joint's action. On every PN and NP command, set both `env.scene.robot.joint_ordering` and `env.scene.robot.body_ordering` to the **source** checkpoint's convention (`physx` for PN, `mjwarp` for NP); `physics=` still selects the target backend. The current `Isaac-Lift-Franka` asset resolves to the same joint and body order in both backends and needs no override; `Isaac-Velocity-Rough-G1` and `Isaac-Velocity-Rough-AnymalD` do need it. Verify by comparing `joint_names` with `backend_joint_names` (and the body equivalents); a scrambled axis collapses the policy immediately rather than degrading gracefully.
...
Require a task trainable in both backends, exact environment-contract equality, one active Franka finger drive in each backend, joint and body ordering resolved to the source checkpoint's convention on every cross-backend replay, matched nominal control behavior, plausible randomization, and all four training/deployment combinations.
