...
``Isaac-Ant-v0``, and ``Isaac-Four-Bar-v0``. Passing ``presets=newton_kamino``
to another task does not automatically enable Kamino; the task must define and
validate its own ``newton_kamino`` preset.
