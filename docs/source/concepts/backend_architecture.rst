...
configs own the implementation class, and their composition roots construct it
with the same convention used by other declarative configs:
...
The config determines the implementation independently of the physics backend.
:class:`~isaaclab.renderers.RenderContext` retains one renderer for each equal
renderer config, while :class:`~isaaclab.sim.SimulationContext` owns visualizer
