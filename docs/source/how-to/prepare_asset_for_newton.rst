...
logic. ``joint_velocity_limit`` requests a solver-side clamp. Although Isaac Lab writes the latter
to Newton's ``Model.joint_velocity_limit``, MJWarp drops it while stepping and Kamino honors it.
When a task needs a speed bound under MJWarp, implement and validate it in task or control logic.
``joint_effort_limit`` is the simulated effort limit; use ``actuator_effort_limit`` to clip an
explicit actuator model, and choose both from the motor and gearbox rather than one oversized
value for the whole robot.
