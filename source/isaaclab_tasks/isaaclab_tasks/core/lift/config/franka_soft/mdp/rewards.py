...
def deformable_fingertip_distance(
    ...
    Nodal analogue of :class:`object_fingertip_distance`: each selected finger body is rewarded for
    approaching the nearest deformable node (:attr:`~isaaclab.assets.DeformableObject.data.nodal_pos_w`),
    ...
    if target_com:
        target_w = wp.to_torch(asset.data.root_pos_w).unsqueeze(1)
    else:
        target_w = wp.to_torch(asset.data.nodal_pos_w)
