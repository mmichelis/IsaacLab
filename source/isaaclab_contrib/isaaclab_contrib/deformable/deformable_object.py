...
class DeformableObject(BaseDeformableObject):
    ...
    def _initialize_impl(self):
        ...
        pattern = re.compile(rf"^(?:{self.cfg.prim_path})(?:/.*)?$")
        try:
            deformable_view = DeformableView(model, pattern, verbose=False)
        except KeyError as error:
            raise RuntimeError(
                f"No Newton-imported deformable body instances found for '{self.cfg.prim_path}'."
            ) from error

        if deformable_view.family not in ("surface", "volume"):
            raise RuntimeError(f"Unsupported deformable type '{deformable_view.family}' for '{self.cfg.prim_path}'.")

        num_envs = SimulationManager.get_num_envs()
        expected_count = deformable_view.count if num_envs is None else num_envs
        if deformable_view.count != expected_count or deformable_view.count_per_world != 1:
            raise RuntimeError(
                f"Expected one deformable body per world for '{self.cfg.prim_path}', found "
                f"{deformable_view.count} bodies across {deformable_view.world_count} worlds."
            )

        self._num_instances = deformable_view.count
        self._particles_per_body = deformable_view.particles_per_group
        self._particle_offsets = deformable_view.starts("particle")
        self._deformable_type = deformable_view.family
